...
class AdminLogger:
    """
    Logger class to redirect console output to log files for the admin module.
    ...
    def __init__(self, log_dir="logs"):
        ...
        self.log_file_path = self.log_dir / f"admin_log_{timestamp}.txt"
        ...
        print(f"[AdminLogger] Logging started. Output will be saved to: {self.log_file_path}")
    ...
    def stop(self):
        ...
        try:
            ...
            print(f"[AdminLogger] Error closing log file: {e}")
        
        print(f"[AdminLogger] Logging stopped. Log file: {self.log_file_path}")
...
def init_logging():
    """Initialize logging and return the logger instance for admin."""
    try:
        return AdminLogger().start()
    except Exception as e:
        print(f"Failed to initialize admin logging: {e}")
        traceback.print_exc()
        return None
...
def log_exception(e, context=""):
    """Log an exception with context information for admin."""
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
...
def log_message(message, level="INFO"):
    """
    Log a message directly to the current admin log file.
