...
class AudioServer:
    ...
    def stream_audio(self, client):
        ...
        try:
            while self.running and not self.receiving_audio:
                try:
        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    # Mark that we're receiving audio (stops sending)
                    self.receiving_audio = True
                    
                    # Play the audio - convert bytearray to bytes
                    try:
                        if playback_stream.is_active():
                            playback_stream.write(bytes(audio_data))
                    except Exception as e:
