...
class MessageHandler:
        ...
        try:
            if msg['type'] == 'room_assignment' and msg['computer_name'] == self.kiosk_app.computer_name:
                ...
                if msg.get('room') == self.kiosk_app.assigned_room:
                    ...
                    print(f"[Kiosk] handle_message: Received hint, clearing hint_requested_flag for room {self.kiosk_app.assigned_room}")
                ...
                if command == "start" and not self.kiosk_app.room_started:  # Correctly check if room is NOT started
                    ...
                    print(f"[Kiosk] handle_message: Timer started, setting room_started to True for room {self.kiosk_app.assigned_room}")
                ...
                if command == "start":
                    self.kiosk_app.hint_requested_flag = False
                    print(f"[Kiosk] handle_message: Timer started, clearing hint_requested_flag for room {self.kiosk_app.assigned_room}")
    ...
    def handle_stop_video_command(self):
        ...
        print("[DEBUG] Received stop video command")
