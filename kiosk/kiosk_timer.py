...
class KioskTimer:
    ...
    def update_display(self):
        """Updates the timer display, checking if a video is playing."""
        ...
        try:
            ...
            self.root.after(0, lambda: self._do_update_display(time_str))

        except Exception as e:
            print(f"[kiosk timer]Error updating timer display: {e}")
            self._update_scheduled = False

    def _do_update_display(self, time_str):
        """Performs the actual display update."""
        try:
